      ...
      <div class="px-6 py-4">
        <div class="space-y-6">
          ...
          <div class="flex items-center space-x-4">
            ...
            <!-- 선호 메뉴 섹션 -->
            <div
              class="flex-1/2 w-full bg-white dark:bg-gray-700 rounded-lg p-4">
              <h5
                class="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-3">
                선호 메뉴
              </h5>
      ...
      <div class="p-6 border-b border-gray-200 dark:border-gray-700">
        ...
        <div class="p-6">
          <p class="text-sm text-gray-900 dark:text-white">
            {{ customerToDel.name }}고객을 삭제 하시겠습니까?
          </p>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
  ...
  },
  {
    ...
    value:
      customers.value.length > 0
        ? Math.round(
            customers.value.reduce((sum, c) => sum + c.reservationCount, 0) /
    ...
    render: (item) => `
      ...
      item.name
        ...
        <div class="ml-4">
          ...
          <div class="text-xs text-gray-400 dark:text-gray-500 mt-1">가입일: ${
            item.joinDate
          }</div>
        </div>
      </div>
...
const customerFilterFn = (data, filters) => {
  ...
  if (filters.statusFilter && filters.statusFilter !== "all") {
    result = result.filter(
      (customer) => customer.status === filters.statusFilter
    );
  }
  return result;
};
