<template>
  <!-- 통계 카드 -->
  <DashboardStats v-if="showStats" :stats="stats" />
  <!-- 기사 현황(공통 컴포넌트 사용) -->
  <SearchTable
    :data="workers"
  ...
  <div
    v-if="selectedWorker"
    @click="closeModal"
    class="modal fixed inset-0 bg-red-500/75 flex items-center justify-center p-4 z-50">
    <div
      @click.stop
...
const saveWorker = () => {
  ...
  if (index !== -1) {
    // workers ref에 바로 저장하면 stats computed가 자동으로 업데이트됨
