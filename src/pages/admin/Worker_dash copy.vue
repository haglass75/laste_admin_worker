...
const saveWorker = () => {
  ...
  const index = workers.findIndex((w) => w.id === selectedWorker.value.id);
  if (index !== -1) {
    const worker = workers[index];
    worker.name = selectedWorker.value.name;
    worker.phone = selectedWorker.value.phone;
    worker.rating = selectedWorker.value.rating;
    worker.status = selectedWorker.value.status;
    if (selectedWorker.value.area !== undefined) {
      worker.area = selectedWorker.value.area;
    }
    if (selectedWorker.value.memo !== undefined) {
      worker.memo = selectedWorker.value.memo;
    }
  }
  closeModal();
};
...
const toggleWokerStatus = (worker) => {
  ...
  if (index !== -1) {
    workers[index].status = worker.status === "활동중" ? "비활성화" : "활동중";
  }
};
// 비활성화 토글기능
