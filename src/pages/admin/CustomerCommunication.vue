<template>
  <div
    ...
    <!-- 필터 및 검색 -->
    <div class="bg-white rounded-lg shadow-sm p-4 mb-6 dark:bg-black dark:text-white">
    ...
    <!-- 대화 목록 -->
    <div class="bg-white rounded-lg shadow-sm dark:bg-black dark:text-white">
      ...
      <div
        ...
        <div class="p-6 border-b flex justify-between items-center">
          ...
          <button
            ...
            @click="selectedConverstaion = null">
...
const avgResponseTime = computed(() => {
  ...
  // reduce((a, b)란 배열의 모든 요소를 더하는 함수이다.
  // a, b 는 배열의 요소이다.
  ...
  return Math.round(times.reduce((a, b) =>
  
  a + b, 0) / times.length);
});
const satisfactionRate = computed(() => {
  const ratings = conversations.value
    .map((c) => c.rating)
    .filter((r) => r !== null);
...
function sendMessage() {
  ...
  // round란 소수점 자리수를 반올림하는 함수이다.
  // (new Date() - selectedConverstaion.value.startDate) / 1000 / 60 는 대화 시작 시간과 현재 시간의 차이를 분으로 변환하는 함수이다.
  // 1000은 1초를 밀리초로 변환하는 함수이다.
  // 60은 1분을 초로 변환하는 함수이다.
  // Math.round 는 소수점 자리수를 반올림하는 함수이다.
  selectedConverstaion.value.responseTime = Math.round(
    (new Date() - selectedConverstaion.value.startDate) / 1000 / 60
  );
  // 분을 시간으로 변경
  newMessage.value = "";
...
function markAnswered() {
  ...
  const idx = conversations.value.findIndex(
    (c) => c.id === selectedConverstaion.value.id
  );
  if (idx !== -1) {
